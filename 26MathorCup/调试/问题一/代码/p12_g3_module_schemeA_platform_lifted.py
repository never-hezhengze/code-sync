# -*- coding: utf-8 -*-
"""
问题1.2：G3 扩展平台模板（近车高抬升版）
文件名建议：p12_g3_module_schemeA_platform_lifted.py

相对 ext 版本的新增点
-------------------
1. 先生成“基础扩展平台模板”：
   - 同类底座平台：1x2, 2x1, 2x2, 1x3, 3x1, 2x3, 3x2
   - 支持单 G3 / 双 G3 共平台模板
2. 再做“模板抬升”：
   - 对当前模板中的底座 G 组合进行竖向复制抬高
   - 保持 footprint（长宽）不变
   - 保持顶部 G3 数量不变
   - 仅增加底座层数，使模板总高度尽量接近车厢有效高度
3. 目标：
   - 让部分模板更接近车厢高度，减少竖向碎片
   - 但仍保持规则长方体模块、实现复杂度可控
...
- 模块仍然只允许落地（z=0）
- 模块上方不再生成空间
- 为防止模板库爆炸，最终只保留评分最好的前 MAX_TEMPLATES_PER_TRUCK 个模板
"""
...
RANDOM_SEED = 42
MAX_TEMPLATES_PER_TRUCK = 60

# 近车高阈值：抬升模板更偏好高度达到车高 75% 以上
...
class ModuleTemplate:
    ...
    volume_bbox: int
    score: float
    source_kind: str = "base"   # base / lifted
...
def base_template_score(truck, base_L, base_W, total_h, total_wt, gl, gw, g3_count):
    ...
    near_top_bonus = 0.08 if height_util >= LIFT_HEIGHT_RATIO else 0.0
...
def generate_base_templates(truck_name: str) -> List[ModuleTemplate]:
                ...
                for g3_ori in ORIENTATIONS["G3"]:
                    ...
                    for layers in range(1, min(max_layers, 4) + 1):
                        ...
                        for g3_count in (1, 2):
                            ...
                            score = base_template_score(
                                truck, base_L, base_W, total_h, total_wt, gl, gw, g3_count
                            )
...
def generate_lifted_templates(truck_name: str, base_templates: List[ModuleTemplate]) -> List[ModuleTemplate]:
    """
    对当前模板中的“G 组合”做抬升：
    - 保持 footprint 不变
    - 保持顶部 G3 数量不变
    - 通过增加 support layers 让高度接近车厢
    """
    truck = TRUCKS[truck_name]
    lifted: List[ModuleTemplate] = []

    for tpl in base_templates:
        # 每增加 1 层，相当于在当前 footprint 下再增加一层底座
        per_layer_support = tpl.pattern_nx * tpl.pattern_ny
        ...
        for new_layers in range(tpl.layers + 1, max_layers + 1):
            ...
            score = base_template_score(
                ...
                tpl.g3_length, tpl.g3_width, tpl.g3_count
            ) + 0.05  # 给抬升模板一点额外偏好
...
def build_g3_platform_templates_for_truck(truck_name: str) -> List[ModuleTemplate]:
    base_templates = generate_base_templates(truck_name)
    lifted_templates = generate_lifted_templates(truck_name, base_templates)

    templates = base_templates + lifted_templates

    # 去重：同 footprint / 构成 / 层数 / G3 数量，保留评分最高
    best_map = {}
    ...
    templates.sort(key=lambda t: (-t.score, -t.height, -t.g3_count, -t.volume_bbox, t.weight))
...
def eval_module_candidate(tpl: ModuleTemplate, sp, truck_obj):
    ...
    lifted_bonus = 0.05 if tpl.source_kind == "lifted" else 0.0
    return 0.52 * fill_ratio + 0.22 * height_bonus + 0.10 * edge_bonus + g3_bonus + lifted_bonus
...
def solve_one_truck_type_with_modules(truck_name):
    ...
    print(f"[{truck_name}] 模板库总数: {len(module_templates)}（base={base_n}, lifted={lift_n}）")
    for tpl in module_templates[:40]:
        print(
            f"  模板 {tpl.name}: "
            ...
            f"重量={tpl.weight:.3f}, 来源={tpl.source_kind}, 配额={module_quota_left.get(tpl.name, 0)}"
        )
...
def solve_for_truck(truck_name, outdir):
    ...
    write_templates_csv(os.path.join(truck_dir, f"{truck_name}_G3抬升平台模板.csv"), module_templates)
...
def parse_args():
    parser = argparse.ArgumentParser(description="问题1.2：G3 抬升平台模板 + 主装箱")
    parser.add_argument("--truck", choices=["车型1", "车型2", "all"], default="all")
    parser.add_argument("--outdir", type=str, default="../结果展示/output1.2_G3抬升平台模板方案A", help="输出目录")
