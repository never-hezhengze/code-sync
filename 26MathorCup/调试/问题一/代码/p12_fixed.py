...
def merge_spaces(spaces: List[Space]) -> List[Space]:
    ...
    for i in range(len(spaces)):
        ...
        for j in range(i + 1, len(spaces)):
            if used[j]:
                continue
            m = try_merge_two_spaces(cur, spaces[j])
            if m is not None:
                merged_once.append(m)
                used[i] = True
                used[j] = True
                merged = True
                break
        ...
        if not merged and not used[i]:
            merged_once.append(cur)
            used[i] = True

    spaces = remove_contained_spaces(merged_once)

    # 控制空间数量，避免 beam search 后面继续爆炸
    spaces = sort_spaces_dblf(spaces)
    MAX_SPACES_AFTER_MERGE = 80
    return spaces[:MAX_SPACES_AFTER_MERGE]
...
def apply_placement(truck: Truck, state: LayoutState, item_idx: int, ori: Orientation, space_idx: int, vehicle_id: str) -> LayoutState:
    ...
    MAX_SPACES_KEEP = 60
    new_spaces = new_spaces[:MAX_SPACES_KEEP]
...
def pack_with_beam_search(
    ...
    step_count = 0
    max_steps = min(len(items) * 2, 120)
    best = init

    while beam and step_count < max_steps:
        step_count += 1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="MathorCup D题 - 问题1.2 物理约束空间切分终极版")
    parser.add_argument("--truck", choices=["车型1", "车型2", "all"], default="all")
    parser.add_argument("--outdir", type=str, default="../结果展示/output1.2(fixed)", help="输出目录")
