# -*- coding: utf-8 -*-
"""
问题1.2：G3 平台模块 + 尾车局部重装版
文件名建议：p12_g3_module_tailrepack.py

思路
----
1. 先用“基础平台模板版”构造整车解；
2. 对最后 k 辆车做局部重装（k=2~4）；
3. 尝试把这 k 辆车中的货物重新打包成更少的车辆：
   - 优先尝试 k -> k-1
   - 若失败，再保持 k 辆但寻找更高质量尾部方案
4. 若成功减少车辆数，则替换尾部解。

说明
----
- 这里选用“基础平台模板版”作为主干，因为它对两种车型都较稳；
- 尾车局部重装是后处理模块，不改动前面已构造好的车辆；
- 模块模板与普通货物都会参与尾部重装。
"""
...
def solution_summary(plans, truck_name):
...
# 尾车局部重装
# =========================================================
def extract_tail_counts(plans: List[FastTruck], k: int) -> Dict[str, int]:
    counts = Counter()
    for p in plans[-k:]:
        counts.update(p.actual_counts)
    return dict(counts)


def build_tail_repack(truck_name: str, counts: Dict[str, int], target_vehicles: int):
    """
    对尾部 counts 做局部重装，目标压成 target_vehicles 辆。
    返回：成功时返回 plans，否则返回 None
    """
    truck = TRUCKS[truck_name]
    ...
    while sum(remaining.values()) > 0 and vehicle_idx <= target_vehicles:
...
def tail_repack_improve(truck_name: str, plans: List[FastTruck], show_progress: bool = True) -> List[FastTruck]:
    """
    依次尝试对最后 4/3/2 辆做局部重装，目标压缩 1 辆。
    例如：
      4 -> 3
      3 -> 2
      2 -> 1
    ...
    while changed:
        ...
        for k in (4, 3, 2):
            ...
            tail_counts = extract_tail_counts(improved, k)
            repacked = build_tail_repack(truck_name, tail_counts, target_vehicles=k - 1)
            ...
            if repacked is not None:
                ...
                # 重新编号尾车
                for i, p in enumerate(repacked, start=1):
                    p.vehicle_id = f"{truck.name}_{len(head) + i:03d}"

                if show_progress:
                    print(f"[{truck_name}] 尾车局部重装成功：最后 {k} 辆 -> {k-1} 辆")

                improved = head + repacked
                changed = True
                break
...
# 主求解
# =========================================================
def solve_one_truck_type_with_tail_repack(truck_name: str, show_tail_repack: bool = True):
    truck = TRUCKS[truck_name]
    ...
    before = len(plans)
    plans = tail_repack_improve(truck_name, plans, show_progress=show_tail_repack)
    after = len(plans)

    if show_tail_repack:
        print(f"[{truck_name}] 尾车重装前车辆数={before}，重装后车辆数={after}")

    summary = solution_summary(plans, truck_name)
    return plans, summary, module_templates


def solve_for_truck(truck_name, outdir, show_tail_repack=True):
    truck = TRUCKS[truck_name]
    plans, summary, module_templates = solve_one_truck_type_with_tail_repack(
        truck_name=truck_name,
        show_tail_repack=show_tail_repack,
    )
...
def parse_args():
    parser = argparse.ArgumentParser(description="问题1.2：G3 平台模块 + 尾车局部重装")
    parser.add_argument("--truck", choices=["车型1", "车型2", "all"], default="all")
    parser.add_argument("--outdir", type=str, default="../结果展示/output1.2_G3平台模块_尾车重装", help="输出目录")
...
def main():
    ...
    for truck_name in truck_list:
        ...
        summary = solve_for_truck(truck_name, outdir, show_tail_repack=True)
        print(summary)
        print()
