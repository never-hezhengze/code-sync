...
"""
问题1.2：G3 平台模块混合模板库版
文件名建议：p12_g3_module_schemeA_platform_hybrid.py
...
1. 车型1：
   使用混合模板库（base + ext + lifted）
   - base：基础平台模板（1x2, 2x1, 2x2，单/双 G3）
   - ext：扩展平台模板（1x3, 3x1, 2x3, 3x2，单/双 G3）
   - lifted：近车高抬升模板
   目标：兼顾前期强吃货与后期收尾灵活性

2. 车型2：
   使用基础平台模板库（base only）
   目标：保持你实验中最好的 13 辆水平，避免大模板和高模板过拟合
                ...
                for g3_ori in ORIENTATIONS["G3"]:
                    ...
                    for layers in range(1, min(max_layers, 4) + 1):
                        ...
                        for g3_count in (1, 2):
                            ...
                            templates.append(
                                ModuleTemplate(
                                    name=f"MPH_{kind}_{support_type}_{nx}x{ny}_L{layers}_S{s_ori.orient_id}_G3{g3_ori.orient_id}_K{g3_count}",
...
def init_module_quota(module_templates: List[ModuleTemplate], remaining: Dict[str, int], truck_name: str) -> Dict[str, int]:
...
def solve_one_truck_type_with_modules(truck_name):
    ...
    print(f"[{truck_name}] 混合模板库总数: {len(module_templates)}（base={base_n}, ext={ext_n}, lifted={lift_n}）")
...
def solve_for_truck(truck_name, outdir):
    ...
    write_templates_csv(os.path.join(truck_dir, f"{truck_name}_G3混合模板库.csv"), module_templates)
...
def parse_args():
    parser = argparse.ArgumentParser(description="问题1.2：G3 混合模板库 + 主装箱")
    parser.add_argument("--truck", choices=["车型1", "车型2", "all"], default="all")
    parser.add_argument("--outdir", type=str, default="../结果展示/output1.2_G3混合模板库方案A", help="输出目录")
