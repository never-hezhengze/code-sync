...
def build_priority_types(inst_seq: List[str]) -> List[str]:
    """把实例序列压缩成类型优先级序列，避免在构造阶段反复扫描同类实例。"""
    out = []
    seen = set()
...
def place_one_grasp(truck_obj, priority_types, remain_counter, start_ptr, priority_window, rcl_size, rnd):
    candidates = []
    spaces_limit = min(len(truck_obj.spaces), MAX_SPACES_TO_CHECK)
    active_types = [t for t in priority_types if remain_counter[t] > 0]
    if not active_types:
        return False, 0

    # 轻量化：只看类型优先级前窗，而不是大量重复实例
    type_window = max(3, min(len(active_types), priority_window))
    candidate_types = active_types[:type_window]
...
def construct_solution_grasp(truck_name, inst_seq, seed, priority_window=80, rcl_size=5):
    ...
    priority_types = build_priority_types(inst_seq)
    plans = []
    start_ptr = 0
    vehicle_idx = 1
    ...
    while sum(remain_counter.values()) > 0:
        ...
        while True:
            ok, start_ptr = place_one_grasp(
                ft, priority_types, remain_counter, start_ptr,
                priority_window=min(priority_window, len(priority_types)),
                rcl_size=rcl_size,
                rnd=rnd,
            )
            if not ok:
...
def parse_args():
    ...
    parser.add_argument("--priority-window", type=int, default=5)
    parser.add_argument("--tabu-iter", type=int, default=6)
    parser.add_argument("--tabu-tenure", type=int, default=6)
    parser.add_argument("--tabu-candidates", type=int, default=6)
    parser.add_argument("--elite-size", type=int, default=4)
    parser.add_argument("--pr-pairs", type=int, default=2)
