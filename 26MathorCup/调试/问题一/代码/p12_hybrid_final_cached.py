...
def revalidate_all(truck, quotas):
    key = tuple(tuple(q[t] for t in TYPE_ORDER_ALL) for q in quotas)

    if key in _REVALIDATE_CACHE:
        return _REVALIDATE_CACHE[key]

    plans = [ga_optimize_vehicle_policy(truck, q, i)[0] for i, q in enumerate(quotas, 1)]

    _REVALIDATE_CACHE[key] = plans
    return plans
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sa-iterations", type=int, default=40, help="每轮压缩的 SA 迭代次数")
