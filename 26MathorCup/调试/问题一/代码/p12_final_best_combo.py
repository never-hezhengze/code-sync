...
"""
问题1.2 最终整合版
文件名建议：p12_final_best_combo.py

当前最优组合（基于实验结果固化）：
- 车型1：使用 hybrid 混合模板库（base + ext + lifted）
- 车型2：使用基础平台模板库（base only）

设计目标：
- 不再强求统一模板策略
- 直接固化当前已验证的最优分车型方案
- 一次运行即可输出两种车型各自的最佳已知组合结果
"""
                ...
                for g3_ori in ORIENTATIONS["G3"]:
                    ...
                    for layers in range(1, min(max_layers, 4) + 1):
                        ...
                        for g3_count in (1, 2):
                            ...
                            templates.append(
                                ModuleTemplate(
                                    name=f"MPF_{kind}_{support_type}_{nx}x{ny}_L{layers}_S{s_ori.orient_id}_G3{g3_ori.orient_id}_K{g3_count}",
...
def build_g3_platform_templates_for_truck(truck_name: str) -> List[ModuleTemplate]:
    """
    最终整合策略：
    - 车型1：hybrid = base + ext + lifted
    - 车型2：base only（复现最佳13辆策略）
    """
...
def solve_one_truck_type_with_modules(truck_name):
    ...
    print(f"[{truck_name}] 最终模板库总数: {len(module_templates)}（base={base_n}, ext={ext_n}, lifted={lift_n}）")
...
def solve_for_truck(truck_name, outdir):
    ...
    write_templates_csv(os.path.join(truck_dir, f"{truck_name}_最终模板库.csv"), module_templates)
...
def parse_args():
    parser = argparse.ArgumentParser(description="问题1.2 最终整合版：分车型最优模板策略")
    parser.add_argument("--truck", choices=["车型1", "车型2", "all"], default="all")
    parser.add_argument("--outdir", type=str, default="../结果展示/output1.2_final_best_combo", help="输出目录")
