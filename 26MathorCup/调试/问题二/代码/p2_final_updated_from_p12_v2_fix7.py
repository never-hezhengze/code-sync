...
def quick_mix_feasible_bounds(p12, x: int, y: int, objective_name: str = "min_vehicle") -> bool:
    """弱剪枝：只保留真正稳妥的必要条件。高概率点进一步放宽。"""
    t1 = p12.TRUCKS["车型1"]
    t2 = p12.TRUCKS["车型2"]
    ...
    # 2) 对高概率点，不再使用 G3/G4/G5 的“纯地板容量”剪枝
    # 因为最新 p12 已经使用模块化、多层结构，地板上界会误杀 (1,12) 这类关键点。
    if is_high_prob_point(x, y, objective_name):
        return True

    # 3) 普通点加一条非常温和的 G3 地板上界，仅用于减少明显不可能点
    g3_need = p12.CARGO_TYPES["G3"].quantity
    g3_cap = (
        x * max_floor_count_for_type(p12, "车型1", "G3")
        + y * max_floor_count_for_type(p12, "车型2", "G3")
    )
    # 放宽到至少能直接承接一部分 G3；其余交给模块化与多层装载
    if g3_cap < int(0.35 * g3_need):
        return False
...
def diagonal_priority_from_boundary(x: int, y: int) -> Tuple[int, int, int]:
    """
    以边界点 (0,13) 为参考，给问题2.2候选点分层优先级：
    - 第一层：主对角线/次对角线优先（即 y 接近 13-x）
    - 第二层：越靠近边界点 (0,13) 越优先
    - 第三层：x 更小优先（更符合“少量车型1替代车型2尾车”）
    """
    diag_y = max(0, 13 - x)
    diag_gap = abs(y - diag_y)
    boundary_dist = abs(x - 0) + abs(y - 13)
...
def solve_problem_22(p12) -> MixedSolution:
    ...
    boundary = try_fixed_point_solution(p12, 0, BOUND_Y2, "min_cost", thorough=True)
    if boundary is None or (not boundary.feasible):
        raise RuntimeError("最新固定点验证器连边界点 (0,13) 都无法通过，请检查验证逻辑。")
