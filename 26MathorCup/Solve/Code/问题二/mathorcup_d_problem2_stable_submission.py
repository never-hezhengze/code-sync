...
def solve_problem_21_min_vehicle(seed: int = RANDOM_SEED, verbose: bool = True) -> List[VehiclePlan]:
    vehicles = build_solution_mixed("min_vehicles", seed=seed, verbose=verbose)
    if verbose:
        print(f"[问题2.1] 初始车辆数={len(vehicles)}")
    vehicles = local_improve_min_vehicles(vehicles, verbose=verbose)
    if verbose:
        print(f"[问题2.1] 最终车辆数={len(vehicles)}")
    return vehicles
...
def solve_problem_22_min_cost(seed: int = RANDOM_SEED + 1, verbose: bool = True) -> List[VehiclePlan]:
    ...
    # 2) 全车型1 构造
    # 3) 全车型2 构造
    cand_solutions: List[List[VehiclePlan]] = []

    cand_solutions.append(build_solution_mixed("min_cost", seed=seed, verbose=verbose))

    def build_pure(truck_name: str, verbose_inner: bool) -> List[VehiclePlan]:
        remaining = initial_remaining()
        vehicles: List[VehiclePlan] = []
        vehicle_idx = 1
        while not all_remaining_zero(remaining):
            if verbose_inner:
                print(f"[纯{truck_name}] 第 {vehicle_idx} 辆车，剩余件数={total_remaining_items(remaining)}")
            plan = build_plan_for_specific_truck(TRUCKS[truck_name], remaining, vehicle_idx, "min_cost")
            if plan.item_count <= 0:
                raise RuntimeError(f"纯{truck_name}构造失败")
            subtract_plan_from_remaining(remaining, plan)
            vehicles.append(plan)
            vehicle_idx += 1
        return vehicles

    cand_solutions.append(build_pure("车型1", verbose_inner=False))
    cand_solutions.append(build_pure("车型2", verbose_inner=False))

    best = min(cand_solutions, key=lambda s: (total_cost(s), len(s), -average_space_util(s)))
    best = local_improve_min_cost(best, verbose=verbose)
    if verbose:
        print(f"[问题2.2] 最终成本={total_cost(best):.2f}，车辆数={len(best)}")
    return best
