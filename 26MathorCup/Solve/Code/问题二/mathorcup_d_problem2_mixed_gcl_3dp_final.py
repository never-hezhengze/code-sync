...
def place_one_best(truck_obj: FastTruck, remaining: Dict[str, int], policy: PackingPolicy) -> bool:
    ...
    for sp_idx, _ in enumerate(candidate_spaces):
        for type_id in type_order:
            ...
            for ori in ORIENTATIONS[type_id]:
                if not truck_obj.can_place_with_physics(type_id, ori, sp_idx):
                    continue
                val = eval_candidate(type_id, ori, truck_obj.spaces[sp_idx], remaining, truck_obj, policy)
                if val is None:
                    continue
...
def ga_search_best_vehicle(
    ...
    for _ in range(generations):
        scored: List[Tuple[Tuple[float, float, float, float], PackingPolicy, VehicleResult]] = []
        for policy in population:
            plan = pack_single_vehicle_with_policy(truck, remaining, policy, vehicle_idx)
            fit = vehicle_fitness(plan, objective)
            scored.append((fit, policy, plan))
            if best_plan is None or fit > best_fit:
                best_plan = plan
                best_fit = fit

        scored.sort(key=lambda x: x[0], reverse=True)
        elites = [p for _, p, _ in scored[:max(2, pop_size // 3)]]

        new_population = elites[:]
        while len(new_population) < pop_size:
            if rng.random() < 0.55:
                pa, pb = rng.sample(elites, 2) if len(elites) >= 2 else (elites[0], elites[0])
                child = crossover_policy(pa, pb, rng)
            else:
                child = elites[rng.randrange(len(elites))]
            if rng.random() < 0.85:
...
def build_initial_solution_mixed(objective: str, seed: int, ga_pop: int, ga_gen: int) -> SolutionResult:
    ...
    while not all_loaded(remaining):
        cand1 = ga_search_best_vehicle(TRUCKS["车型1"], remaining, vehicle_idx, objective, seed + vehicle_idx * 101 + 1, ga_pop, ga_gen)
        cand2 = ga_search_best_vehicle(TRUCKS["车型2"], remaining, vehicle_idx, objective, seed + vehicle_idx * 101 + 2, ga_pop, ga_gen)
        best = compare_vehicle_candidates(cand1, cand2, objective)
...
def construct_solution_with_vehicle_limit(
    ...
    for idx in range(1, max_vehicle_count + 1):
        ...
        cand1 = ga_search_best_vehicle(TRUCKS["车型1"], remaining, idx, objective, seed + idx * 131 + 1, ga_pop, ga_gen)
        cand2 = ga_search_best_vehicle(TRUCKS["车型2"], remaining, idx, objective, seed + idx * 131 + 2, ga_pop, ga_gen)
        best = compare_vehicle_candidates(cand1, cand2, objective)
...
def solve_problem_21_min_vehicle(seed: int, ga_pop: int, ga_gen: int, attempts_per_k: int) -> SolutionResult:
    ...
    best.note = f"min_vehicle solved, lower_bound={lb}"
    return best
...
def solve_problem_22_min_cost(seed: int, ga_pop: int, ga_gen: int, attempts_per_k: int) -> SolutionResult:
    ...
    for k in range(lb, upper_k + 1):
        feasible_candidates: List[SolutionResult] = []
        for att in range(attempts_per_k):
            trial = construct_solution_with_vehicle_limit(
                max_vehicle_count=k,
                objective="min_cost",
                seed=seed + k * 2003 + att * 131,
                ga_pop=ga_pop,
                ga_gen=ga_gen,
            )
            if trial.success:
        ...
        cur_best = feasible_candidates[0]
        if best is None or (cur_best.total_cost, cur_best.vehicle_count) < (best.total_cost, best.vehicle_count):
            best = cur_best

    if best is None:
        raise RuntimeError("成本最优搜索失败，未找到任何可行混合车型解。")

    best.note = f"min_cost solved, lower_bound={lb}, upper_k={upper_k}"
    return best
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="MathorCup D题 - 问题2 混合车型 GCL-3DP 最终版")
    ...
    parser.add_argument("--ga-pop", type=int, default=12, help="单车 GA 种群规模")
    parser.add_argument("--ga-gen", type=int, default=6, help="单车 GA 演化代数")
    parser.add_argument("--attempts", type=int, default=4, help="每个 K 或每个阶段的重试次数")
