...
def lower_bound_vehicle_count_mix(p12) -> int:
    """
    用车型2的体积/载重/G3底层能力给出问题2的粗下界。
    这样可以避免从 (0,0)、(1,0) 这种明显不可能的点开始验证。
    """
...
def generate_candidates_for_problem_21(p12) -> List[Tuple[int, float, int, int]]:
    """
    问题2.1：按“车辆数 -> 成本 -> (x,y)”排序。
    只保留：
    1. 车辆数不小于理论下界；
    2. 车辆数严格小于纯车型2边界 13 的点；
       因为 (0,13) 已作为保底可行边界，没必要再重复验证 >=13 的点。
    """
...
def generate_candidates_for_problem_22(p12) -> List[Tuple[float, int, int, int]]:
    """
    问题2.2：按“成本 -> 车辆数 -> (x,y)”排序。
    只保留：
    1. 车辆数不小于理论下界；
    2. 成本严格小于纯车型2边界 9100 的点；
       因为 (0,13) 已作为保底可行边界，没必要再重复验证 >=9100 的点。
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--outdir",
        type=str,
        default="output_problem2_point_enumeration_strong",
        help="输出目录",
    )
