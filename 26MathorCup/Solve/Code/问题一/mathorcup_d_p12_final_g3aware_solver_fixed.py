...
# -*- coding: utf-8 -*-
"""
MathorCup D题 - 问题1.2 最终版（修正版）
G3主导下界 + 分层分配 + 单车增强装箱 + 守恒型轻量LNS修复

修正点：
1. 自动扩大搜索范围时，从上次结束的 K 继续，而不是从 LB 重新开始；
2. LNS 修复仅允许“跨车搬运/交换”，保持每类货物总量守恒，避免目标货量膨胀；
3. G3 作为易碎件，只允许底板放置，用底面积下界参与车辆数估计；
4. 输出每辆车装箱明细和车辆汇总。

说明：
- 这是工程化近似算法，重视稳定可运行和结果可解释性；
- 若 K 很小时无可行解，程序会自动继续扩大搜索范围；
- 单车内部使用轻量 beam search + DBLF + 三空间分割 + 空间合并。
"""
...
def build_item_orders(items: List[CargoItem], policy: int) -> List[CargoItem]:
    ...
    # 1.2 强化：优先 G3，其次 G4/G5，再用 G1/G2 填缝
    if policy == 0:
        return sorted(items, key=lambda it: (it.type_id != "G3", it.type_id not in ("G4", "G5"), -base_area(it), -it.volume, it.item_id))
    if policy == 1:
        return sorted(items, key=lambda it: (not it.fragile, not it.oriented, -it.weight, -it.volume, it.item_id))
    if policy == 2:
...
def empty_quota_list(K: int) -> List[Dict[str, int]]:
    return [{t: 0 for t in ALL_TYPES} for _ in range(K)]


def quota_weight(q: Dict[str, int]) -> float:
    return sum(q[t] * CARGO_TYPES[t].weight for t in ALL_TYPES)


def quota_volume(q: Dict[str, int]) -> int:
    return sum(q[t] * CARGO_TYPES[t].volume for t in ALL_TYPES)
...
def total_quota_counts(quotas: List[Dict[str, int]]) -> Dict[str, int]:
    total = {t: 0 for t in ALL_TYPES}
    for q in quotas:
        for t in ALL_TYPES:
            total[t] += q.get(t, 0)
    return total
...
def validate_total_conservation(quotas: List[Dict[str, int]]) -> None:
    total = total_quota_counts(quotas)
    required = {t: CARGO_TYPES[t].quantity for t in ALL_TYPES}
    if total != required:
        raise RuntimeError(f"配额守恒被破坏: {total} != {required}")


def initial_quota_split_g3_aware(truck: Truck, K: int) -> List[Dict[str, int]]:
    quotas = empty_quota_list(K)

    # 1) G3 先均匀分配，且不超过单车 G3 地板容量
    g3_total = CARGO_TYPES["G3"].quantity
    g3_cap = floor_capacity_for_type(truck, "G3")
    base = g3_total // K
    extra = g3_total % K
    for j in range(K):
        quotas[j]["G3"] = min(base + (1 if j < extra else 0), g3_cap)

    assigned = sum(q["G3"] for q in quotas)
    remain = g3_total - assigned
    idx = 0
    while remain > 0:
    ...
    # 2) G4/G5 分配给 G3 较少、剩余体积/重量更大的车
    for t in ["G4", "G5"]:
        remain = CARGO_TYPES[t].quantity
        while remain > 0:
            best_idx = max(
                range(K),
                key=lambda j: (
        ...
        while remain > 0:
            best_idx = max(
                ...
                key=lambda j: (
                    ...
                    residual_weight(truck, quotas[j]) >= CARGO_TYPES[t].weight,
                    min(
                        residual_volume(truck, quotas[j]) / max(1, CARGO_TYPES[t].volume),
                        residual_weight(truck, quotas[j]) / max(1, CARGO_TYPES[t].weight)
                    ),
                    residual_volume(truck, quotas[j]),
                )
            )
    ...
    validate_total_conservation(quotas)
    return quotas
...
def collect_unpacked(plans: List[VehiclePlan]) -> Dict[str, int]:
    missing = {t: 0 for t in ALL_TYPES}
    for vp in plans:
        for t in ALL_TYPES:
...
def move_units(quotas: List[Dict[str, int]], src: int, dst: int, t: str, delta: int) -> int:
    if src == dst or delta <= 0:
        return 0
    can = min(delta, quotas[src][t])
    if can <= 0:
        return 0
    quotas[src][t] -= can
    quotas[dst][t] += can
...
def repair_with_lns(truck: Truck, quotas: List[Dict[str, int]], beam_width: int = 3, max_rounds: int = 6, rng_seed: int = 2026) -> Tuple[List[VehiclePlan], bool]:
    ...
    for rd in range(max_rounds):
        ...
        # 1) 优先修 G3：从 G3 过多且装不下的车，搬到 G3 有底面积余量的车
        g3_donors = sorted(
            range(len(plans)),
            key=lambda j: quotas[j]["G3"] - plans[j].actual_counts.get("G3", 0),
            reverse=True,
        )
        g3_receivers = sorted(
            range(len(plans)),
            key=lambda j: (vehicle_g3_slack(truck, plans[j]), residual_capacity_counts(truck, plans[j]).get("G3", 0)),
            reverse=True,
        )
        for d in g3_donors:
            overflow = quotas[d]["G3"] - plans[d].actual_counts.get("G3", 0)
            if overflow <= 0:
                continue
            for r in g3_receivers:
                if r == d:
                    continue
                slack = min(vehicle_g3_slack(truck, plans[r]), residual_capacity_counts(truck, plans[r]).get("G3", 0))
                if slack <= 0:
                    continue
                moved = move_units(quotas, d, r, "G3", min(overflow, slack, 2))
                overflow -= moved
                if overflow <= 0:
                    break

        # 2) 修 G4/G5：从“目标大于实际”的车搬到大件较少且剩余体积更大的车
        for t in ["G4", "G5"]:
            donors = sorted(
                range(len(plans)),
                key=lambda j: quotas[j][t] - plans[j].actual_counts.get(t, 0),
                reverse=True,
            )
            receivers = sorted(
                range(len(plans)),
                key=lambda j: (quotas[j]["G4"] + quotas[j]["G5"], -residual_volume(truck, quotas[j]), -residual_weight(truck, quotas[j])),
            )
            for d in donors:
                overflow = quotas[d][t] - plans[d].actual_counts.get(t, 0)
                if overflow <= 0:
                    continue
                for r in receivers:
                    if r == d:
                        continue
                    if residual_weight(truck, quotas[r]) < CARGO_TYPES[t].weight:
                        continue
                    if residual_volume(truck, quotas[r]) < CARGO_TYPES[t].volume:
                        continue
                    moved = move_units(quotas, d, r, t, 1)
                    overflow -= moved
                    if overflow <= 0:
                        break

        # 3) 小件平衡：只允许交换/搬运，不允许净增加
        for _ in range(6):
            a, b = rng.sample(range(len(quotas)), 2)
            ta = rng.choice(["G1", "G2"])
            tb = rng.choice(["G1", "G2"])
            move_units(quotas, a, b, ta, 2)
            move_units(quotas, b, a, tb, 2)
...
def solve_problem_12_fixed_truck(
    truck: Truck,
    beam_width: int = 3,
    lns_rounds: int = 6,
    max_extra_k: int = 8,
    start_k: Optional[int] = None,
) -> Tuple[int, List[VehiclePlan], Dict[str, object]]:
    ...
    print(f"{truck.name} 在 K={start_k}~{end_k} 范围内未找到解，自动扩大搜索范围...", flush=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="MathorCup D题 - 问题1.2 最终版（修正版）")
    ...
    parser.add_argument("--outdir", type=str, default="output_p12_final_g3aware_fixed", help="输出目录")
