...
# 尾车替换触发阈值
LAST_BIG_TRUCK_SV_THRESHOLD = 0.65
LAST_BIG_TRUCK_FS_THRESHOLD = 0.58
...
def quota_volume(quota: Dict[str, int]) -> int:
    return sum(CARGO_TYPES[t].volume * quota.get(t, 0) for t in TYPE_ORDER_ALL)


def quota_weight(quota: Dict[str, int]) -> float:
...
def solve_mixed_fleet_greedy(objective: str) -> Tuple[List[VehicleResult], Dict[str, object]]:
    ...
    return finalize_summary(plans, objective)
...
def is_better_after_tail_replace(
    old_plans: List[VehicleResult],
    new_plans: List[VehicleResult],
    objective: str,
) -> bool:
    old_stats = solution_stats(old_plans)
    new_stats = solution_stats(new_plans)

    if objective == "min_vehicle":
        key_old = (
            old_stats["vehicle_count"],
            old_stats["total_cost"],
            -old_stats["avg_space_utilization"],
        )
        key_new = (
            new_stats["vehicle_count"],
            new_stats["total_cost"],
            -new_stats["avg_space_utilization"],
        )
...
def quick_check_tail_to_small(last_plan: VehicleResult) -> bool:
    """
    快速必要条件：总体积、总重量不能明显超过车型1能力
    """
    truck1 = TRUCKS["车型1"]
    if last_plan.used_volume > truck1.effective_volume:
        return False
    if last_plan.used_weight > truck1.max_weight:
        return False
    return True


def try_replace_last_big_truck_with_small(
    plans: List[VehicleResult],
    objective: str,
) -> List[VehicleResult]:
    if not plans:
    ...
    last_plan = plans[-1]
    if last_plan.truck_name != "车型2":
        return plans

    tr2 = TRUCKS["车型2"]
    sv2, wv2, fs2 = truck_score(tr2, last_plan.used_volume, last_plan.used_weight)

    # 尾车不够空，则不尝试
    if not (sv2 < LAST_BIG_TRUCK_SV_THRESHOLD or fs2 < LAST_BIG_TRUCK_FS_THRESHOLD):
        return plans

    # 快速必要条件
    if not quick_check_tail_to_small(last_plan):
        print(
            f"[尾车替换] {last_plan.vehicle_id} 利用率偏低，但体积/重量已超过车型1上限，跳过。"
        )
        return plans

    tail_quota = {t: int(last_plan.actual_counts.get(t, 0)) for t in TYPE_ORDER_ALL}
    if sum(tail_quota.values()) == 0:
        return plans

    print(
        f"[尾车替换] 检查 {last_plan.vehicle_id}："
        f"空间利用率={sv2:.4f}，fullness_score={fs2:.4f}，"
        f"尝试改用1辆车型1承接尾货。"
    )

    candidate_small = build_one_vehicle_best(TRUCKS["车型1"], tail_quota, 999)

    # 必须把尾货全部装完
    if candidate_small.actual_counts != {k: v for k, v in tail_quota.items() if v > 0}:
        # 为了稳妥，改成逐类比较
        feasible = True
        for t in TYPE_ORDER_ALL:
            if candidate_small.actual_counts.get(t, 0) != tail_quota.get(t, 0):
                feasible = False
                break
        if not feasible:
            print("[尾车替换] 车型1无法独立装下尾车全部货物，保持原方案。")
            return plans

    new_plans = plans[:-1] + [candidate_small]

    if is_better_after_tail_replace(plans, new_plans, objective):
        sv1, wv1, fs1 = truck_score(TRUCKS["车型1"], candidate_small.used_volume, candidate_small.used_weight)
        print(
            f"[尾车替换] 成功：{last_plan.vehicle_id}(车型2) -> {candidate_small.vehicle_id}(车型1)，"
            f"新尾车空间利用率={sv1:.4f}，fullness_score={fs1:.4f}"
        )
        return new_plans

    print("[尾车替换] 虽可装下，但目标未改善，保持原方案。")
    return plans


def finalize_summary(plans: List[VehicleResult], objective: str) -> Tuple[List[VehicleResult], Dict[str, object]]:
    plans2 = try_replace_last_big_truck_with_small(plans, objective)
    summary = solution_stats(plans2)
    summary["objective"] = objective
    return plans2, summary
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="MathorCup D题 - 问题二（重构版：快速贪心混合车型 + 尾车替换）")
    parser.add_argument("--objective", choices=["min_vehicle", "min_cost", "both"], default="both")
    parser.add_argument("--outdir", type=str, default="output_problem2_greedy_mixed_tail_replace")
...
def main() -> None:
    ...
    for obj in objectives:
        print("=" * 72)
        print(f"开始求解问题二（重构版 + 尾车替换），目标 = {obj}")
        print("=" * 72)
