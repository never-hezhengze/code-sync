...
def count_score(truck: Truck, counts: Dict[str, int]) -> float:
    ...
    densest = max(c.weight / c.volume for c in CARGO_TYPES.values())
...
def placement_gain(truck: Truck, state: LayoutState, item: CargoItem, ori: Orientation, space: Space) -> float:
    ...
    dense_bonus = 0.03 * (item.weight / item.volume) / max(c.weight / c.volume for c in CARGO_TYPES.values())
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--truck", type=str, default="车型1", choices=list(TRUCKS.keys()), help="选择车型")
    parser.add_argument("--topk", type=int, default=18, help="进入第二阶段验证的候选计数组合数")
    parser.add_argument("--beam-width", type=int, default=8, help="第二阶段 beam width")
    parser.add_argument("--trials", type=int, default=1200, help="第一阶段随机贪心试验次数")
...
def main():
    ...
    print(f"""
    【{truck.name}结果】
    空间利用率: {meta['space_utilization']:.4f}
    载重利用率: {meta['weight_utilization']:.4f}
    综合满载率: {meta['fullness_score']:.4f}
    装载件数: {meta['placed_item_count']}
    """)
