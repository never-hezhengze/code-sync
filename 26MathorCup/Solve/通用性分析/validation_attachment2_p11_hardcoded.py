...
# 附件2通用性检验（问题1.1专用，硬编码数据版）
# 算法：Count-Layout Beam Heuristic for 3D Packing
# 设定：
# 1. 每种箱装产品数量相同，统一取 100 件；
# 2. 忽略重量相关约束；
# 3. 全部按标准件处理；
# 4. 只选典型封闭车型；
# 5. 不读取 Excel，货物与车型数据直接写在代码中。
...
def generate_count_candidates(truck: Truck, upper_bounds: Dict[str, int], trials: int = 120, keep: int = 18, seed: int = 2026) -> List[Dict[str, int]]:
    ...
    for trial in range(trials):
        ...
        while attempts < 2000:
            ...
            for t in TYPE_ORDER:
                ...
                new_counts[t] += 1
                score = count_score(truck, new_counts) + rng.uniform(-0.01, 0.01)
...
def pack_with_beam_search(truck: Truck, items: List[CargoItem], beam_width: int = 4, candidate_items_each_step: int = 5, candidate_moves_each_state: int = 8, policy: int = 0) -> LayoutState:
    ...
    while beam:
        ...
        for state in beam:
            ...
            for item_idx, item in enumerate(item_candidates):
                for space_idx, space in enumerate(state.spaces[: min(12, len(state.spaces))]):
...
def evaluate_candidate(truck: Truck, counts: Dict[str, int], beam_width: int):
    ...
    for policy in range(4):
        state = pack_with_beam_search(truck, items, beam_width=beam_width, candidate_items_each_step=5, candidate_moves_each_state=8, policy=policy)
...
def solve_problem_11(truck: Truck, topk: int = 6, beam_width: int = 4, trials: int = 120, seed: int = 2026):
    upper_bounds = compute_upper_bounds(truck)
    candidates = generate_count_candidates(truck, upper_bounds, trials=trials, keep=max(topk * 2, 12), seed=seed)
    best_state = None
    best_meta = {}
...
def main():
    ...
    outdir = os.path.join(base_dir, "附件2_通用性检验结果_硬编码版")
    ...
    for truck_name, truck in TRUCKS.items():
        ...
        state, meta, _ = solve_problem_11(truck, topk=6, beam_width=4, trials=120, seed=2026)
        result = {
