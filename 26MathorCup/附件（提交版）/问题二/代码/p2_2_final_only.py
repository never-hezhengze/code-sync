...
def is_low_loaded_type2_plan(plan, p12) -> bool:
    truck2 = p12.TRUCKS["车型2"]
    if truck2.length * truck2.width * truck2.effective_height <= 0:
        return False
    vol_util = plan.used_volume / (truck2.length * truck2.width * truck2.effective_height)
    wt_util = plan.used_weight / truck2.max_weight
    return vol_util <= 0.60 or wt_util <= 0.55


def build_exact_truck1_repack(p12, type2_plan) -> Optional[object]:
    counts = {t: int(type2_plan.actual_counts.get(t, 0)) for t in type_order_all(p12)}
    if sum(counts.values()) == 0:
        return None

    truck1 = p12.TRUCKS["车型1"]
    templates1 = build_vehicle_templates_full(p12, "车型1")
    quota1 = p12.init_module_quota(templates1, counts.copy(), "车型1")
    plan1, _ = p12.build_one_vehicle_best_with_modules(
        truck=truck1,
        remaining=counts,
        vehicle_idx=1,
        module_templates=templates1,
        module_quota_left=quota1,
        truck_name="车型1",
    )
    if plan1 is None:
        return None
    if sum(plan1.actual_counts.values()) != sum(type2_plan.actual_counts.values()):
        return None
    for t, v in type2_plan.actual_counts.items():
        if plan1.actual_counts.get(t, 0) != int(v):
            return None
    return plan1


def try_low_loaded_type2_to_type1(p12, solution: MixedSolution) -> Optional[MixedSolution]:
    if solution.y_truck2 <= 0:
        return None

    for plan in solution.plans:
        if not str(plan.vehicle_id).startswith("车型2_"):
            continue
        if not is_low_loaded_type2_plan(plan, p12):
            continue

        repacked = build_exact_truck1_repack(p12, plan)
        if repacked is None:
            continue

        new_plans = [p for p in solution.plans if p.vehicle_id != plan.vehicle_id]
        repacked.vehicle_id = f"车型1_{solution.x_truck1 + 1:03d}"
        new_plans.append(repacked)

        return MixedSolution(
            objective_name=solution.objective_name,
            x_truck1=solution.x_truck1 + 1,
            y_truck2=solution.y_truck2 - 1,
            plans=new_plans,
            feasible=True,
            construction_mode=solution.construction_mode + "+低载车型2->车型1转换",
        )
...
def solve_problem_22(p12) -> MixedSolution:
    ...
    improved_boundary = try_low_loaded_type2_to_type1(p12, boundary)
    if improved_boundary is not None:
        print(f"[问题2.2] 直接在边界解上将低载车型2替换为车型1，得到更优点 ({improved_boundary.x_truck1},{improved_boundary.y_truck2})", flush=True)
        return improved_boundary
    ...
    for cost, vehicles, x, y in candidates:
        ...
        if sol is not None and sol.feasible:
            improved = try_low_loaded_type2_to_type1(p12, sol)
            if improved is not None:
                print(f"[问题2.2] 低载车型2可由车型1替换，找到更优点 ({improved.x_truck1},{improved.y_truck2})", flush=True)
                return improved
    ...
    with open(path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for vp in solution.plans:
            ...
            w.writerow([
                ...
                vp.used_volume,
                len(getattr(vp, "placed_modules", [])),
            ])


def build_module_template_map(p12) -> Dict[str, object]:
    templates: Dict[str, object] = {}
    try:
        for tpl in p12.build_templates_hybrid_for_truck1():
            templates[tpl.name] = tpl
    except Exception:
        pass
    try:
        for tpl in p12.build_templates_base_only_for_truck2():
            templates[tpl.name] = tpl
    except Exception:
        pass
    return templates
...
def expand_module_items(module, template_map, p12) -> List[Dict[str, object]]:
    tpl = template_map.get(module.template_name)
    if tpl is None:
        raise RuntimeError(f"无法找到模块模板: {module.template_name}")

    support_type = tpl.support_type
    support_ori = next(
        ori for ori in p12.ORIENTATIONS[support_type] if ori.orient_id == tpl.support_orientation_id
    )
    g3_ori = next(
        ori for ori in p12.ORIENTATIONS["G3"] if ori.orient_id == tpl.g3_orientation_id
    )
    ...
    direct_load: Dict[str, float] = {}

    for layer in range(tpl.layers):
        z0 = module.z + layer * sh
        for ix in range(tpl.pattern_nx):
            x0 = module.x + ix * sl
            for iy in range(tpl.pattern_ny):
                y0 = module.y + iy * sw
                support_by = None if layer == 0 else lower_map[(layer - 1, ix, iy)]
                item_id = f"{support_type}_{module.module_id}_{layer+1:02d}_{ix+1:02d}_{iy+1:02d}"
                cargo = p12.CARGO_TYPES[support_type]
                items.append({
                    "record_type": "item",
                    "item_id": item_id,
                    ...
                    "y": y0,
                    "z": z0,
                    "length": support_ori.size[0],
                    "width": support_ori.size[1],
                    "height": support_ori.size[2],
                    "weight": cargo.weight,
                    "rotation": support_ori.rotation,
                    "orientation_id": support_ori.orient_id,
                    "truck_id": module.truck_id,
                    "support_by": support_by or "",
                    "direct_supported_weight": 0.0,
                    "template_name": tpl.name,
                })
                lower_map[(layer, ix, iy)] = item_id

    if tpl.g3_count == 1:
        g3_positions = [(module.x, module.y)]
    elif tpl.g3_count == 2:
        if 2 * gl <= tpl.length:
            g3_positions = [(module.x, module.y), (module.x + gl, module.y)]
        elif 2 * gw <= tpl.width:
            g3_positions = [(module.x, module.y), (module.x, module.y + gw)]
        else:
            raise RuntimeError(f"模块模板 {tpl.name} 无法展开为 2 个 G3 布局。")
    else:
        g3_positions = []

    top_support_ids = [
        lower_map[(tpl.layers - 1, ix, iy)]
        for ix in range(tpl.pattern_nx)
        for iy in range(tpl.pattern_ny)
    ]
    support_by_str = ";".join(top_support_ids)
    for idx, (x0, y0) in enumerate(g3_positions, start=1):
        item_id = f"G3_{module.module_id}_{idx:02d}"
        cargo = p12.CARGO_TYPES["G3"]
        items.append({
            "record_type": "item",
            "item_id": item_id,
            "type_id": "G3",
            ...
            "x": x0,
            "y": y0,
            "z": module.z + tpl.layers * sh,
            "length": g3_ori.size[0],
            "width": g3_ori.size[1],
            "height": g3_ori.size[2],
            "weight": cargo.weight,
...
def write_expanded_items_csv(path: str, plan, p12) -> None:
    os.makedirs(os.path.dirname(path), exist_ok=True)
    template_map = build_module_template_map(p12)
    with open(path, "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow([
        ...
        for p in getattr(plan, "placed", []):
            w.writerow([
                ...
                p.support_by or "",
                round(getattr(p, "direct_supported_weight", 0.0), 6),
                "",
            ])

        for module in getattr(plan, "placed_modules", []):
            for item in expand_module_items(module, template_map, p12):
                w.writerow([
                    item["record_type"],
                    item["item_id"],
                    item["type_id"],
                    ...
                    item["orientation_id"], item["rotation"], item["truck_id"],
                    item["support_by"],
                    round(item["direct_supported_weight"], 6),
                    item["template_name"],
                ])
...
def write_solution_outputs(outdir: str, tag: str, p12, solution: MixedSolution) -> Dict[str, object]:
    ...
    for vp in solution.plans:
        write_expanded_items_csv(os.path.join(sol_dir, f"{vp.vehicle_id}_装箱明细.csv"), vp, p12)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--p12-path",
        type=str,
        default="../../问题一/代码/p12_final_expanded_check.py",
        help="最新问题1.2求解器路径",
    )
    parser.add_argument(
        "--outdir",
