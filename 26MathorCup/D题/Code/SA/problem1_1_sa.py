...
def make_neighbor(sequence: List[CargoItem], rng: random.Random, current_result: PackingResult) -> List[CargoItem]:
    child = sequence[:]
    n = len(child)
    if n < 2:
        return child
...
def simulated_annealing_optimize_single_truck(
    ...
    print(f"[{truck.name}] SA开始：loaded={len(best_result.placed)}, score={best_result.score:.6f}")

    while temp > final_temp:
        for _ in range(iters_per_temp):
            step += 1
            candidate_seq = make_neighbor(current_seq, rng, current_result)
...
def solve_problem_1_1_sa(alpha: float = 0.5, beta: float = 0.5) -> Dict[int, PackingResult]:
    items = expand_cargo_items(load_cargo_types())
    results = {}
    for truck_id, truck in load_truck_types().items():
        print(f"开始使用 SA 求解 {truck.name} ...")
        results[truck_id] = simulated_annealing_optimize_single_truck(
            items,
            truck,
            alpha=alpha,
            beta=beta,
...
def save_problem_1_1_sa_results(results: Dict[int, PackingResult], output_dir: Path | None = None) -> None:
    ...
    for truck_id, result in results.items():
        ...
        (output_dir / f"problem1_1_sa_truck_{truck_id}_solution.json").write_text(
            json.dumps(data, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
    (output_dir / "problem1_1_sa_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )


def print_problem_1_1_sa_summary(results: Dict[int, PackingResult], elapsed: float) -> None:
    print("问题1.1 单车满载率最大化（SA）求解结果")
    print("=" * 60)
...
if __name__ == "__main__":
    start = time.time()
    results = solve_problem_1_1_sa()
    save_problem_1_1_sa_results(results)
    print_problem_1_1_sa_summary(results, time.time() - start)