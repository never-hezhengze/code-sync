...
def make_sa_neighbor(sequence: List[CargoItem], rng: random.Random, current_result: PackingResult) -> List[CargoItem]:
    ...
    if move == "swap":
        ...
        if current_result.unplaced:
            promote = rng.sample(current_result.unplaced, k=min(len(current_result.unplaced), rng.randint(1, 4)))
            ...
            pos = rng.randint(0, min(12, len(others)))
            child = others[:pos] + chosen + others[pos:]

    return child


def destroy_sequence(sequence: List[CargoItem], result: PackingResult, rng: random.Random, ratio: float = 0.18) -> Tuple[List[CargoItem], List[CargoItem]]:
    """
    LNS destroy:
    从序列中移除一部分货物，优先考虑难装货、大件、以及未装入货物附近货物
    """
    n = len(sequence)
    remove_count = max(3, min(n // 3, int(n * ratio)))
    ...
    for item in hard_sorted[: max(remove_count * 2, 12)]:
        candidate_ids.add(item.item_id)
...
def greedy_repair_sequence(
    ...
    for item in removed_items:
        ...
        # 候选插入位置：全遍历会慢，这里做稀疏采样
        candidate_positions = list(range(0, len(seq) + 1))
        if len(candidate_positions) > 30:
            step = max(1, len(candidate_positions) // 20)
            candidate_positions = candidate_positions[::step]
            if len(seq) not in candidate_positions:
                candidate_positions.append(len(seq))
...
def sa_lns_optimize_single_truck(
    ...
    print(f"[{truck.name}] SA+LNS开始：loaded={len(best_result.placed)}, score={best_result.score:.6f}")
    ...
    while temp > final_temp:
        ...
        if temp_round % lns_every == 0:
            partial_seq, removed_items = destroy_sequence(current_seq, current_result, rng, ratio=0.18)
            repaired_seq = greedy_repair_sequence(partial_seq, removed_items, truck, alpha, beta)
...
def solve_problem_1_1_sa_lns(alpha: float = 0.5, beta: float = 0.5) -> Dict[int, PackingResult]:
    items = expand_cargo_items(load_cargo_types())
    results = {}

    for truck_id, truck in load_truck_types().items():
        print(f"开始使用 SA + LNS 求解 {truck.name} ...")
...
def save_problem_1_1_sa_lns_results(results: Dict[int, PackingResult], output_dir: Path | None = None) -> None:
    ...
    for truck_id, result in results.items():
        ...
        (output_dir / f"problem1_1_sa_lns_truck_{truck_id}_solution.json").write_text(
            json.dumps(data, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
    (output_dir / "problem1_1_sa_lns_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )


def print_problem_1_1_sa_lns_summary(results: Dict[int, PackingResult], elapsed: float) -> None:
    print("问题1.1 单车满载率最大化（SA + LNS）求解结果")
    print("=" * 60)
...
if __name__ == "__main__":
    start = time.time()
    results = solve_problem_1_1_sa_lns()
    save_problem_1_1_sa_lns_results(results)
    print_problem_1_1_sa_lns_summary(results, time.time() - start)