...
def unique_orientations(size: Tuple[int, int, int], category: str) -> Tuple[Orientation, ...]:
    l, w, h = size
    perms = {
        "standard": [((l, w, h), "LWH"), ((l, h, w), "LHW"), ((w, l, h), "WLH"), ((w, h, l), "WHL"), ((h, l, w), "HLW"), ((h, w, l), "HWL")],
...
def expand_cargo_items(cargo_types: Dict[str, CargoType]) -> List[CargoItem]:
    ...
    for cargo in cargo_types.values():
        ...
        for idx in range(1, cargo.quantity + 1):
            items.append(CargoItem(f"{cargo.type_id}_{idx:03d}", cargo.type_id, cargo.category, size, cargo.weight, volume, cargo.stackable, cargo.fragile, cargo.oriented, orientations))
    return items
...
def boxes_overlap(x: int, y: int, z: int, dims: Tuple[int, int, int], p: PlacedCargo) -> bool:
    l, w, h = dims
    return overlap(x, x + l, p.x, p.x + p.length) and overlap(y, y + w, p.y, p.y + p.width) and overlap(z, z + h, p.z, p.z + p.height)
...
def can_place(item: CargoItem, ori: Orientation, point: Tuple[int, int, int], truck: TruckType, placed: List[PlacedCargo], current_weight: float):
    x, y, z = point
    l, w, h = ori.size
...
def candidate_points(placed: List[PlacedCargo]) -> List[Tuple[int, int, int]]:
    pts = {(0, 0, 0)}
    for p in placed:
        pts.update({(p.x + p.length, p.y, p.z), (p.x, p.y + p.width, p.z), (p.x, p.y, p.z + p.height)})
    valid_points = [pt for pt in pts if not any(point_inside_box(pt, p) for p in placed)]
    layers: Dict[int, List[Tuple[int, int, int]]] = {}
    for pt in valid_points:
        layers.setdefault(pt[2], []).append(pt)
    result: List[Tuple[int, int, int]] = []
    for z in sorted(layers):
        layer_points = sorted(layers[z], key=lambda t: (t[0] + t[1], t[0], t[1]))
        result.extend(layer_points[:MAX_POINTS_PER_LAYER])
        if len(result) >= MAX_CANDIDATE_POINTS:
            break
    return result[:MAX_CANDIDATE_POINTS]
...
def placement_score(item: CargoItem, ori: Orientation, point: Tuple[int, int, int], truck: TruckType, alpha: float, beta: float) -> float:
    ...
    footprint_bonus = 0.005 * (l * w) / (truck.length * truck.width)
    return alpha * volume_ratio + beta * weight_ratio + boundary_bonus + footprint_bonus - compact_penalty
...
def pack_single_truck(items: Sequence[CargoItem], truck: TruckType, alpha: float = 0.5, beta: float = 0.5) -> PackingResult:
    ...
    for item in items:
        best = None
        for point in candidate_points(placed):
        ...
        placed_item = PlacedCargo(item.item_id, item.type_id, item.category, x, y, z, ori.size[0], ori.size[1], ori.size[2], item.weight, ori.orient_id, ori.rotation, support_id, cap)
...
def initial_sequence(items: Sequence[CargoItem], rng: random.Random) -> List[CargoItem]:
    seq = list(items)
    seq.sort(key=lambda item: (item.fragile, -item.weight / item.volume, -item.volume, -item.weight, item.type_id, item.item_id))
...
def genetic_optimize_single_truck(items: Sequence[CargoItem], truck: TruckType, alpha: float = 0.5, beta: float = 0.5, population_size: int = GA_POPULATION_SIZE, generations: int = GA_GENERATIONS, seed: int = RANDOM_SEED) -> PackingResult:
    if len(items) <= 1:
    ...
    best = None
    elite_count = max(2, int(population_size * GA_ELITE_RATIO))
    for g in range(generations):
        print(f"第{g+1}代开始", end="\r")
        scored = []
...
def solve_problem_1_1(alpha: float = 0.5, beta: float = 0.5) -> Dict[int, PackingResult]:
    items = expand_cargo_items(load_cargo_types())
    return {truck_id: genetic_optimize_single_truck(items, truck, alpha, beta) for truck_id, truck in load_truck_types().items()}


def save_problem_1_1_results(results: Dict[int, PackingResult], output_dir: Path | None = None) -> None:
    ...
    for truck_id, result in results.items():
        ...
        (output_dir / f"problem1_1_truck_{truck_id}_solution.json").write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    (output_dir / "problem1_1_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")


def print_problem_1_1_summary(results: Dict[int, PackingResult], elapsed: float) -> None:
    print("问题1.1 单车满载率最大化求解结果")
    print("=" * 60)
