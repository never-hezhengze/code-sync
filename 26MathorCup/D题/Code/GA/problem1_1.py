...
def candidate_points(placed: List[PlacedCargo]) -> List[Tuple[int, int, int]]:
    ...
    return sorted([pt for pt in pts if not any(point_inside_box(pt, p) for p in placed)], key=lambda t: (t[2], t[0] + t[1], t[0], t[1]))
...
def genetic_optimize_single_truck(items: Sequence[CargoItem], truck: TruckType, alpha: float = 0.5, beta: float = 0.5, population_size: int = 18, generations: int = 24, seed: int = RANDOM_SEED) -> PackingResult:
    if len(items) <= 1:
