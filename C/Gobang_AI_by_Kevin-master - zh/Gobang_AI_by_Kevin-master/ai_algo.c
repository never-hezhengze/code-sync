...
#define swapChoice(x, y) do{struct Choice tp=x; x=y; y=tp;}while(0)
#define ifSwapChoice(x, y) if(x.prior < y.prior) swapChoice(x, y)

struct Choice tp[GRID_N];
...
int killSearch(Chessid NowPlayer, int depth, int alpha, int beta)
{
	...
	struct Choice AvaChoices[GRID_N*GRID_N];
...
int heuriSearchChoices( struct Choice* AvaChoices )
{
...
int killSearchChoices( struct Choice* AvaChoices )
{
...
void quickSort(struct Choice *a, int n)
{
	if ( n<=1 )
		return;
...
void merge(struct Choice *a, int n_left, int n_right)
{
	...
	memcpy(a, tp, sizeof(struct Choice)*cnt);
}

void mergeSort(struct Choice *a, int n)
{
	if(n<=1) return;
