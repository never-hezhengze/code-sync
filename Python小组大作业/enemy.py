...
class Enemy(pygame.sprite.Sprite):
    ...
    def setup_enemy_stats(self):
        ...
        if self.enemy_type == "fast":
            ...
            self.health = 2
            self.size = 28
            self.color = settings.COLORS['enemy_sniper']
            self.score_value = 25
            ...
            self.health = 3
            self.size = 32
            self.color = settings.COLORS['enemy_bomber']
            self.score_value = 35
            self.attack_damage = 25
            self.explosion_radius = 80
        elif self.enemy_type == "boss":
            self.speed = settings.ENEMY_SPEED * 0.5
            self.health = 15
            self.size = 60
            self.color = settings.COLORS['enemy_boss']
    ...
    def create_enemy_appearance(self):
        ...
        if self.enemy_type == "fast":
            ...
            pygame.draw.polygon(self.image, self.color, points)
            # 瞄准镜
            pygame.draw.circle(self.image, (255, 255, 255), (center, center), 6, 2)
            pygame.draw.circle(self.image, (255, 255, 255), (center, center), 3, 1)
            # 十字线
            pygame.draw.line(self.image, (255, 255, 255), (center-4, center), (center+4, center), 1)
            pygame.draw.line(self.image, (255, 255, 255), (center, center-4), (center, center+4), 1)
            ...
            # 爆炸符号
            pygame.draw.circle(self.image, (255, 255, 0), (center, center), center - 6, 3)
            # 闪电符号
            points = [
                (center - 6, center - 8),
                (center + 2, center - 2),
            ...
            pygame.draw.circle(self.image, self.color, (center, center), center - 2)
            # 中层装甲
            pygame.draw.circle(self.image, (150, 0, 150), (center, center), center - 8, 4)
            # 内层核心
            pygame.draw.circle(self.image, (255, 0, 255), (center, center), center - 16)
            # 能量线条
            for angle in range(0, 360, 45):
                x = center + math.cos(math.radians(angle)) * (center - 12)
                y = center + math.sin(math.radians(angle)) * (center - 12)
                pygame.draw.line(self.image, (255, 255, 255), (center, center), (x, y), 2)
    ...
    def update(self, player, game_map=None):
        ...
        if game_map:
            # 电力网失效时，电子敌人减速
            if not game_map.is_power_grid_active() and self.enemy_type in ["sniper", "bomber"]:
                speed_modifier = 0.5  # 减速50%
        ...
        if self.enemy_type == "sniper":
            self.sniper_behavior(player, speed_modifier)
        elif self.enemy_type == "bomber":
            self.bomber_behavior(player, speed_modifier)
        elif self.enemy_type == "boss":
            self.boss_behavior(player, speed_modifier)
        else:
    ...
    def sniper_behavior(self, player, speed_modifier=1.0):
        """狙击手行为：保持距离并瞄准"""
        direction = Vector2(player.rect.center) - Vector2(self.rect.center)
        distance = direction.length()
    ...
    def bomber_behavior(self, player, speed_modifier=1.0):
        """爆破手行为：冲向玩家"""
        direction = Vector2(player.rect.center) - Vector2(self.rect.center)
        distance = direction.length()
        ...
        if distance > 0:
            direction = direction.normalize()
            # 爆破手移动更激进
            self.try_move(direction * self.speed * 1.2 * speed_modifier)

    def boss_behavior(self, player, speed_modifier=1.0):
        """Boss行为：复杂的移动模式"""
        direction = Vector2(player.rect.center) - Vector2(self.rect.center)
        distance = direction.length()
        ...
        if distance > 0:
            ...
            # Boss有圆形移动模式
            current_time = pygame.time.get_ticks()
            circle_movement = Vector2(
                math.sin(current_time * 0.002) * 2,
                math.cos(current_time * 0.002) * 2
            )
            
            final_direction = direction + circle_movement
            if final_direction.length() > 0:
                final_direction = final_direction.normalize()
            
            self.try_move(final_direction * self.speed * speed_modifier)
    ...
    def draw_enemy_ui(self, screen, enemy):
        ...
        if enemy.enemy_type == "sniper":
            ...
            if hasattr(enemy, 'attack_range'):
                pygame.draw.circle(screen, (128, 0, 128, 50), 
                                 enemy.rect.center, enemy.attack_range, 1)
        
        elif enemy.enemy_type == "bomber":
            # 爆破手爆炸范围提示
            if hasattr(enemy, 'explosion_radius'):
                # 闪烁的爆炸范围
                current_time = pygame.time.get_ticks()
                if (current_time // 500) % 2:  # 每500ms闪烁
                    pygame.draw.circle(screen, (255, 165, 0, 100), 
                                     enemy.rect.center, enemy.explosion_radius, 2)
        
        elif enemy.enemy_type == "boss":
            # Boss特殊效果
            current_time = pygame.time.get_ticks()
            # 能量光环
            radius = enemy.size // 2 + math.sin(current_time * 0.01) * 5
            pygame.draw.circle(screen, (255, 0, 255, 100), 
                             enemy.rect.center, int(radius), 3)
            
            # Boss血条（更大更显眼）
            bar_width = enemy.size + 20
            bar_height = 8
            bar_x = enemy.rect.centerx - bar_width // 2
