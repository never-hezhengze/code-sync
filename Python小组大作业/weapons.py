...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, start_pos, target_pos, weapon_type=WeaponType.PISTOL):
        ...
        if weapon_type == WeaponType.PISTOL:
            self._init_pistol()
        elif weapon_type == WeaponType.MACHINE_GUN:
            self._init_machine_gun()
        elif weapon_type == WeaponType.CANNON:
            self._init_cannon()
    ...
    def _init_machine_gun(self):
        """初始化机关枪子弹属性"""
        self.image = pygame.Surface((BULLET_SIZE - 2, BULLET_SIZE + 4), pygame.SRCALPHA)
        pygame.draw.ellipse(self.image, (200, 200, 50),  # 浅黄色子弹
                            (0, 0, BULLET_SIZE - 2, BULLET_SIZE + 4))
        self.speed = BULLET_SPEED * 1.2  # 稍快一些
        self.damage = BULLET_DAMAGE * 0.5  # 伤害减半
        self.size = BULLET_SIZE

    def _init_cannon(self):
        """初始化炮筒子弹属性"""
        ...
        for i in range(6):
            ...
            points.append((x, y))
        pygame.draw.polygon(self.image, (255, 100, 100), points)
        self.speed = BULLET_SPEED * 0.6  # 速度较慢
        self.damage = BULLET_DAMAGE * 2.0  # 伤害加倍
        self.size = size
