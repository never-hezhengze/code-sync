...
- 上界比（且不能优于）$E(θ̂_{ls})$，但即使在 $d ≫ n$ 时也能使用。
  （参考：Bartlett-Long-Lugosi-Tsigler 2019，Hastie-Montanari-Rosset-Tibshirani 2019）
- 对于随机设计需要更多工作（Hsu-Kakade-Zhang 2014）。

**高概率界**：对任意 $δ \in (0, 1)$，以概率 $1−δ$ 成立：
$∥θ̂_{ls} − θ*∥²_Σ \lesssim σ²(r + \log(1/δ))/n$。

证明思路：
- $∥Y − Xθ̂_{ls}∥₂² \le ∥Y − Xθ*∥₂² = ∥ε∥₂²$。
- $∥Y − Xθ̂_{ls}∥₂² = ∥Xθ* + ε − Xθ̂_{ls}∥₂² = ∥Xθ̂_{ls} − Xθ*∥₂² − 2εᵀX(θ̂_{ls} − θ*) + ∥ε∥₂²$。
- 得 $∥X(θ̂_{ls} − θ*)∥₂² \le 2εᵀX(θ̂_{ls} − θ*)$。
- 设 $Φ = [ϕ₁, …, ϕ_r] \in ℝⁿˣʳ$ 为 $X$ 列空间的标准正交基。存在 $ν \in ℝʳ$ 使 $X(θ̂_{ls} − θ*) = Φν$。
- $εᵀX(θ̂_{ls} − θ*)/∥X(θ̂_{ls} − θ*)∥₂ = εᵀΦν/∥Φν∥₂ = ε̃ᵀν/∥ν∥₂ \le \sup_{u\in B₂} ε̃ᵀu$，其中 $ε̃ = Φᵀε$。
- 由于 $\forall u \in S^{r−1}$，$P[ε̃ᵀu > t]$ 有 sub-Gaussian 尾，得：
  以概率至少 $1−δ$：$∥X(θ̂_{ls} − θ*)∥₂ \le 2 \sup_{u\in B₂} ε̃ᵀu \le √(8σ²r) + √(4σ² \log(1/δ))$。

当 $r = d$ 时：$∥θ̂_{ls} − θ*∥₂² \lesssim σ²(d + \log(1/δ))/(n λ_{\min}(Σ))$，以高概率成立。
...
- **$K = B₁$（$ℓ₁$ 球）**：有 $2d$ 个顶点，$X$ 的列经过归一化使得 $∥Xᵢ∥₂ ≤ √n$。
  ...
  - $ε̃ᵢ = Xᵢᵀε ∼ sub G_d(nσ²)$。
  - $P(sup_{u∈K} ε̃ᵀu > nt) \le 2d exp(−nt²/(2σ²))$。令 $t = O(σ √(log(d/δ)/n))$。
  - $∥θ̂_{ls} − θ*∥²_Σ$ 的证明同样适用于 $∥θ̂_K − θ*∥²_Σ$。因此以高概率：
    $∥θ̂_K − θ*∥²_Σ \lesssim min{σ²(r + log(1/δ))/n, σ √(log(d/δ)/n)}$。

- **$K = B₀(k) = {θ ∈ ℝᵈ : ∥θ∥₀ ≤ k}$**：
  以概率至少 $1−δ$：$∥θ̂_K − θ*∥²_Σ \lesssim (σ²/n)(k log(ed/2k) + k log 6 + log(1/δ))$。
  - 设 $A_k = supp(θ̂_K − θ*)$，则 $|A_k| \le 2k$。设 $Φ = [ϕ₁, …, ϕ_{r_k}]$ 为 $X_{A_k}$ 列空间的标准正交基，$r_k = rank(X_{A_k}) \le 2k$。
  - $∥X(θ̂_K − θ*)∥₂² \le 2 max_{A_k : r_k \le 2k} sup_{v∈B₂} ε̃ᵀ_{A_k} v$。
...
考虑 $Y = Xθ* + ϵ$，$ϵ ∼ N(0, σ²I)$，且 $n \ll d$。

- $θ̂_K$（$K = B₀(k)$）几乎与 Oracle 估计量一样好，仅多出一个 $log d$ 的代价（因为支持集未知），且对 $X$ 不做任何假设。
- 计算 $min_{θ∈B₀(k)} (1/2n)∥Y − Xθ∥₂²$ 的计算复杂度：
  $Σ_{ℓ=1}^k C_d^ℓ (ℓ³ + n²ℓ) \ge O(d^k n² k)$。（NP 难问题）

- 因此考虑 **Lasso**：$θ̂_L = arg min_{θ∈ℝᵈ} {(1/2n)∥Y − Xθ∥₂² + λ∥θ∥₁}$。

- 不失一般性，假设 $X$ 的列经过 $√n$-归一化。
- 令 $λ* = σ √(2 log(2d/δ)/n)$，则 $∥θ̂_L − θ*∥²_Σ \lesssim ∥θ*∥₁ σ √(log(d/δ)/n)$，以概率至少 $1−δ$ 成立。

  证明思路：
  - $(1/2n)∥Y − Xθ̂_L∥₂² + λ∥θ̂_L∥₁ \le (1/2n)∥Y − Xθ*∥₂² + λ∥θ*∥₁$。
  - $∥θ̂_L − θ*∥²_Σ \le ⟨Xᵀε/n, θ̂_L − θ*⟩ + λ(∥θ*∥₁ − ∥θ̂_L∥₁)$。
  - 定义事件 $A = {∥Xᵀε/n∥_∞ \le λ*}$，取 $λ \ge λ*$。在 $A$ 上：$∥θ̂_L − θ*∥²_Σ \le 2λ∥θ*∥₁$。验证 $P(A) \ge 1−δ$。

- $θ̂_L$ 的预测误差比 $θ̂_K$ 慢（对于强信号）。当 $X$ 满足受限特征值条件时可以得到更快的收敛率。

- **受限特征值条件**：若 $γ = inf_{v∈C} (vᵀXᵀX v)/(n∥v∥²₂) > 0$，其中 $C = {v : ∥v_{A*^c}∥₁ \le 3∥v_{A*}∥₁}$，$A* = supp(θ*)$。
  取 $λ = 2σ √(2 ln(2d/δ)/n)$，则以概率至少 $1−δ$：
  $∥θ̂_L − θ*∥₂ \le (6/γ) σ √(2 ln(2d/δ) |A*|/n)$。
...
## 第 15-16 页：统计误差的界定（Bound $E_{sta}$）

**假设**：
- $F$ 以 $B$ 为界（bounded by $B$）。
- $ℓ(f, Z)$ 关于 $f$ 满足 $Lip(λ)$（$λ$-Lipschitz）。
- $L$ 在 $f*$ 处局部强凸：$L(f) − L(f*) \ge c∥f − f*∥²_{L²(μ_X)}$，对于 $f \in Ball_B(f*)$。

**关键不等式**：
- 若 $E[|W|] < ∞$，则 $E[W] \le ∫_0^∞ P[W > w] dw$。
- $P[(1/n) Σᵢ G(f̃, Zᵢ) > t] \le P[max_{f̃∈F} (1/n) Σᵢ G(f̃, Zᵢ) > t]$。

**覆盖数（Covering number）**：
令 $N(F, ∥·∥_{L∞}, δ)$ 为 $F$ 关于 $δ$-覆盖的覆盖数，覆盖 $C = {f₁, …, f_N}$。

$∀ f̃ ∈ F$，存在 $f̃ ∈ C$ 使得 $|g(f̃, Z) − g(f̃, Z)| \le λδ$，且 $G(f̃, Z) \le G(f̃, Z) + 3λδ$。

**Bernstein 不等式用于有界随机变量**：
对 $∀ f̃ ∈ C$，$|g(f̃, Zᵢ)| \le 2λB$，$|g(f̃, Zᵢ) − E[g(f̃, Zᵢ)]| \le 4λB ≜ b$。

令 $σ² := Var(g(f̃, Zᵢ)) \le E[g(f̃, Zᵢ)²] \le λ² E[|f̃ − f*|²] \le (λ²/c) E[g(f̃, Zᵢ)]$。

通过细致的尾概率分析得到：
$E_{sta} \le ((16λB + 4λ²/c)(log N + 1) + 3λ)/n$。
...
## 第 18 页：$E_{sta}$ 改进泛化误差（$E_{sta}$ improves the generalization error）
...
且 $E_{sta} ≤ ((16λB + 4λ²/c)(log N + 1) + 3λ)/n$。
...
我们想界定近似误差 $E_{app} = inf_{f∈F} ∥f − f*∥²_{L²(μ_X)}$。
...
$μ = Law(a₀Z + b₀X₁)$，$Z ∼ N(0,I)$，$ν = γ_{d,σ} * ρ$。
