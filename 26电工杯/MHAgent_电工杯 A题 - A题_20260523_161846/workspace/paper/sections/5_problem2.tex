...
问题二将园区制氨产能增至72吨/日，电氢氨装置仅有全额开机和停机两种运行方式。需要确定不同产量水平（72、63、54、45、36吨/日，共五种）下成本最低的生产时段安排。产能扩容后设备参数线性提升：ALKEL 20 MW、PEMEL 20 MW、合成氨装置1.5 MW，电氢氨总额定功率$P_{H2NH3}^{max} = 41.5$ MW。每种产量对应的开机时数为$k = Q_{NH3} / 3.0$小时。
...
本问题的核心是在24个时段中选择$k$个时段开机运行，使得日运行总成本最小（图\ref{fig:flow_q2}）。由于每个时段的开机边际成本可独立计算且时段间无耦合约束（无最小开停机时间等约束），采用贪心算法即可获得全局最优解\cite{ref2}。贪心策略的最优性证明如下：目标函数可分解为各时段独立贡献之和，选取边际成本最低的$k$个时段等价于求解原问题的最优解。
...
其中$C_{on}(t)$为时段$t$开机时的运行成本（含购电/售电和运维），$C_{off}(t)$为停机时的成本。具体地：
...
贪心策略为：按$\Delta C(t)$从小到大排序，选取前$k$个时段开机。该策略的时间复杂度为$O(24\log 24)$，远优于MILP的指数级最坏复杂度，且在本问题结构下可证明获得全局最优解。
...
从吨氨成本角度看，产量越低则吨氨成本越低，36吨/日对应最低吨氨成本3191.67元/吨。这是因为低产量方案可以将开机时段集中在风光出力充足且电价低的时段，大幅减少购电需求。然而从绿电指标角度看，低产量方案的指标严重不达标：36吨/日方案的$R_{self}=10.5\%$远低于60\%要求，$R_{grid}=44.8\%$远超20\%上限，原因在于开机时段过短导致大量余电上网。
...
将6种风电出力水平与4种光伏出力水平组合，形成24种风光出力场景。对每种场景分别应用贪心算法求解五种产量水平的最优时段安排，选取吨氨成本最低的方案作为该场景的最优运行策略。

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth,height=0.5\textheight,keepaspectratio]{../figures/fig_q2_scenarios_heatmap.pdf}
\caption{24种风光场景$\times$5种产量水平吨氨成本热力图}\label{fig:q2_scenarios_heatmap}
\end{figure}

24种场景与五种产量水平组合下的吨氨成本分布呈现显著差异（图\ref{fig:q2_scenarios_heatmap}）。热力图横轴为日产氨量（72至36吨/日），纵轴为24种风光出力场景。可以观察到：高风电、高光伏场景（如W1P1）在各产量水平下的吨氨成本均较低；低产量方案在所有场景下成本均低于高产量方案，但绿电指标不达标。风电出力水平对成本的影响大于光伏，这与风电装机容量（40 MW）相对于光伏（64 MW）的出力稳定性有关。
...
全年吨氨成本分布曲线（图\ref{fig:q2_annual_cost}）显示，24种场景的吨氨成本从1370.40元/吨到7125.16元/吨不等，全年加权平均吨氨成本为3283.98元/吨，全年总产量为9720吨。成本分布呈现较大离散度，反映了风光出力随机性对园区经济性的显著影响。
