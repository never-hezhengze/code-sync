...
问题四研究园区离网运行模式，即切断与外部电网的连接，园区用能完全依赖风光发电。离网条件下$P_{buy}(t) = 0$、$P_{sell}(t) = 0$，功率平衡约束更加严格——任何时刻用电负荷不得超过风光发电功率。分三个子问题逐步深入：无储能时的最大产量分析、最优储能配置方案设计\cite{ref6}、离网与联网模式的经济性对比。
...
在模型求解时，沿用问题三的CBC求解器中的分支割平面算法（Branch-and-Cut）最大化全天制氨负荷率之和，从而最大化产氨量。

\subsubsection{计算结果}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth,height=0.27\textheight,keepaspectratio]{../figures/fig_q4_offgrid_production.pdf}
\caption{24种风光场景下离网制氨日产量对比}\label{fig:q4_offgrid_production}
\end{figure}

对24种风光场景分别求解LP，得到各场景的最大制氨产量和弃电量。如图\ref{fig:q4_offgrid_production}所示，24种场景下的离网制氨产量差异显著。高风光场景下日产量可接近72吨上限，而低风光场景下日产量大幅下降。全年关键指标为：全年总产量9782吨，平均产能利用率37.7\%。相比联网模式（问题三）的12960吨/年，离网无储能模式的年产量减少24.5\%，产能利用率不足四成，大量产能因风光时序限制而闲置。
...
为实现园区基本能源自给（覆盖常规负荷加10\%制氢氨最低功率），在所有24种场景最严苛时段约束下，设风电装机容量为$P_{wind}^{cap}$，光伏装机容量为$P_{pv}^{cap}$，风电和光伏在场景$s$、时刻$t$的单位装机出力分别为$p_{s,t}^w$, $p_{s,t}^{pv}$。为了保证园区至少能够支撑常规负荷和制氢氨装置 10\% 最低运行负荷，新增约束为：
\begin{equation}
P_{wind}^{cap}p_{s,t}^{w} + P_{pv}^{cap}p_{s,t}^{pv} \geq P_{load} + 0.1P_{H2NH3}^{max}
\end{equation}

目标为最小化风光装机容量，以发电成本作为权值得到如下目标函数：
\begin{equation}
\min (c_{wind}P_{wind}^{cap}+c_{pv}P_{pv}^{cap})
\end{equation}

决策变量为$P_{wind}^{cap}$和$P_{pv}^{cap}$。这是一个连续线性规划问题(LP)，采用 SciPy 中 linprog 函数的 HiGHS 方法求解线性规划模型，得到最小风电装机容量约为155.0 MW，此时仅靠风电即可满足需求（最小光伏装机为0 MW）。该结果表明，在当前风光资源条件下，风电的出力稳定性优于光伏，是离网园区能源自给的关键支撑。

\subsection{问题四(2)：储能配置优化}

\subsubsection{储能状态方程}
为了描述储能系统的状态，引入储能充放电状态变量$w_{s,t} \in \{0,1\}$，取1时代表储电，取0时代表放电。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.4\textheight,keepaspectratio]{tikz_storage_model-new.pdf}
\caption{储能系统SOC状态转移方程示意图}\label{fig:tikz_storage}
\end{figure}

转移过程如图\ref{fig:tikz_storage}所示，每个时段的荷电量由前一时段的自损耗、充电输入和放电输出三部分共同决定。

\subsubsection{参数扫描优化}

针对最大弃电场景（W4P1），采用参数扫描法对储能容量$C_{sto} \in \{10, 50, 100, 150, 200\}$ MWh进行逐一求解。对每一种固定的储能容量，以最大化日制氨量为目标函数构建混合整数线性规划模型（MILP），求解生产调度方案。分析储能容量对产量提升和弃电减少的边际效果。功率容量比固定为$P_{sto}^{max} = C_{sto} / 2$（0.5C倍率）。

储能系统的吨氨成本需计入投资成本的日均摊分。根据附件6，储能设备投资成本为1000元/kWh，使用寿命15年，则日均摊投资成本为：
\begin{equation}\label{eq:sto_invest}
C_{sto,inv}^{day} = \frac{C_{sto} \times 1000 \times c_{inv}}{T_{life} \times 365} = \frac{C_{sto} \times 10^6}{15 \times 365}
\end{equation}

其中$C_{sto}$为储能容量（MWh），$c_{inv} = 1000$元/kWh为单位投资成本，$T_{life} = 15$年为使用寿命。以最优配置150 MWh为例，日均摊投资成本为$150 \times 10^6 / 5475 = 27397$元/日。

储能系统的总日运行成本为：
\begin{equation}
C_{sto}^{day} = C_{sto,inv}^{day} + c_{om} \cdot E_{throughput} \cdot 1000
\end{equation}

其中$c_{om} = 0.01$元/kWh为储能运维成本，$E_{throughput}$为日充放电吞吐量（MWh）。
\vspace*{-\baselineskip}
\begin{table}[H]
\centering
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth,height=0.28\textheight,keepaspectratio]{../figures/fig_q4_storage_dispatch.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth,height=0.25\textheight,keepaspectratio]{111.png}
