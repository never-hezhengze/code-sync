...
算法：贪心算法——按各时段净成本排序，优先选择净成本最低的时段开机
"""
import numpy as np
import json
import sys, os
...
def solve_q2_greedy(P_wind, P_pv, P_load, Q_target, params, buy_price, sell_price):
    """贪心算法求解问题二单个(场景,产量)组合
    策略：计算每个时段开机的边际净成本，按净成本从低到高贪心选取k个时段开机
    """
    ...
    for t in range(24):
        # 开机时净负荷 = 基础负荷 + H2NH3 - 风光
        net_load_on = P_load[t] + P_H2NH3 - P_wind[t] - P_pv[t]
        # 不开机时净负荷 = 基础负荷 - 风光
        net_load_off = P_load[t] - P_wind[t] - P_pv[t]

        if net_load_on > 0:
            cost_on = buy_price[t] * net_load_on * 1000 + C_OM_hour
        else:
            cost_on = -sell_price * (-net_load_on) * 1000 + C_OM_hour

        if net_load_off > 0:
            cost_off = buy_price[t] * net_load_off * 1000
        else:
            cost_off = -sell_price * (-net_load_off) * 1000

        marginal_costs[t] = cost_on - cost_off

    # 按边际成本排序，选取最低的k个时段开机
    sorted_hours = np.argsort(marginal_costs)
    u_sol = np.zeros(24)
    u_sol[sorted_hours[:k]] = 1.0

    # 计算功率平衡和各指标
    P_buy_sol = np.zeros(24)
    P_sell_sol = np.zeros(24)
    for t in range(24):
        net = P_load[t] + u_sol[t] * P_H2NH3 - P_wind[t] - P_pv[t]
        if net > 0:
            P_buy_sol[t] = net
        else:
            P_sell_sol[t] = -net
...
def solve_problem2():
    ...
    # 问题二(1): 典型场景
    print("\n--- 问题二(1): 典型场景各产量最优方案（贪心算法）---")
    typical_results = {}

    for Q in productions:
        result = solve_q2_greedy(P_wind_typ, P_pv_typ, P_load, Q, params, buy_price, sell_price)
        if result is None:
    ...
    scenario_results = {}   # 存储每个场景各产量的完整结果
    for s in range(24):
        label = labels[s]
        ...
        best_q = None
        for Q in productions:
            result = solve_q2_greedy(wind_all[s], pv_all[s], P_load, Q, params, buy_price, sell_price)
            if result is None:
                continue
            scenario_results[label][str(Q)] = result
            on_hours = [t for t in range(24) if result["u"][t] > 0.5]
            print(f"  Q={Q}t/d: k={result['k']}h, C_ton={result['C_ton']:.2f}")
            print(f"    on_hours={on_hours}")
            print(f"    R_self={result['R_self']*100:.1f}% R_green={result['R_green']*100:.1f}% R_grid={result['R_grid']*100:.1f}%")
            if result["C_ton"] < best_cost:
                ...
                best_q = Q
        scenario_results[label]["best_Q"] = best_q
        scenario_results[label]["best_C_ton"] = round(best_cost, 2)
        if (s + 1) % 8 == 0:
    ...
    # 构建24场景×5产量的吨氨成本矩阵（用于热力图）
    cost_matrix_data = []
    for s in range(24):
        label = labels[s]
        row = {}
        for Q in productions:
            if str(Q) in scenario_results[label]:
                row[str(Q)] = scenario_results[label][str(Q)]["C_ton"]
            else:
                row[str(Q)] = None
        cost_matrix_data.append({"scenario": label, "costs": row})


    scenarios_detailed = {}
    for label in labels:
        scenarios_detailed[label] = scenario_results[label]  

    all_results = {
        "typical": typical_results,
        "best_Q_typical": int(best_Q),
        "scenarios": scenario_summary,                     
        "scenarios_detailed": scenarios_detailed,
        "productions": productions,
        "cost_matrix": cost_matrix_data,
        "annual_stats": {
