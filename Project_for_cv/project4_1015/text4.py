...
def main():
	...
	cv2.imwrite(os.path.join(out_dir, 'text4_gray.jpg'), gray)
	cv2.imwrite(os.path.join(out_dir, 'text4_roberts_magnitude.jpg'), mag_viz)
	cv2.imwrite(os.path.join(out_dir, 'text4_roberts_edge.jpg'), edge_bw)
	...
	cv2.putText(combined, 'Roberts magnitude', (w + gap + 10, 25), font, font_scale, (0, 0, 255), thickness, cv2.LINE_AA)
	cv2.putText(combined, 'Edges (binary)', (2 * (w + gap) + 10, 25), font, font_scale, (0, 0, 255), thickness, cv2.LINE_AA)

	combined_path = os.path.join(out_dir, 'text4_combined.jpg')
	cv2.imwrite(combined_path, combined)

	print(f"已保存: {out_dir}/text4_gray.jpg, text4_roberts_magnitude.jpg, text4_roberts_edge.jpg")
